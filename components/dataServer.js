import { graphs } from "./BackendParameter.js";
import { graphs_phase2 } from "./BackendParameter.js";
...
function Graph_2(data) {
  return getResult(data, function calc(t, patient) {
    ...
    var a3 =
      1 -
      Math.exp(
        -patient.ke * (t > patient.infusion_rate ? patient.infusion_rate : t)
      );
    var a4 =
      t > patient.infusion_rate
    ...
    for (var i = 0; i <= maxHour; i += TimeInterval) {
      ...
      data.forEach((val, j) => {
        var conc = calc(
          i,
          val,
          i === 0 ? 0 : result[result.length - 1][j + 1],
          j
        );
        ave += conc / 20;
        rowData.push(conc);
